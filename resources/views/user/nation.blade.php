@extends('user.master')
@section('title','Phim '.$nationname->nation_name.' - Movie')
@section('content')
            ...
            <div class="browse-agile-w3ls general-w3ls">
                <div class="tittle-head">
                    <h4 class="latest-text">
                        Phim {{$nationname->nation_name}}
                    </h4>
                    <div class="container">
                        <div class="agileits-single-top">
                            <ol class="breadcrumb">
                                <li><a href="{{ route('user.index') }}">Trang Chủ</a></li>
                                <li class="active">Phim {{$nationname->nation_name}}</li>
                            </ol>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="col-md-2 w3l-movie-gride-agile">
                            <a href="{{route('user.movie',$item->id)}}" title="{{$item->vie_name.' ('.$item->eng_name.')'}}"
                                class="hvr-shutter-out-horizontal"><img
                                    src="{{'storage/app/poster/'.$item->poster_image}}" title="{{$item->vie_name.' ('.$item->eng_name.')'}}"
