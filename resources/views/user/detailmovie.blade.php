            ...
            <div class="col-md-4">
                <div class="moviedetailinfor">
                    ...
                    <div class="moviedetailinfor2">
                        ...
                        @foreach ($nation as $item)
                        @if ($item->id==$movie->nation_id)
                        <p><b>Quốc gia: </b><a href="{{ route('user.nation',$movie->nation_id) }}">{{$item->nation_name}}</a></p>
                        @endif
                        @endforeach
                        @foreach ($year as $item)
                        @if ($item->id==$movie->year_id)
                        <p><b>Năm sản xuất: </b><a href="{{ route('user.year',$movie->year_id) }}">{{$item->year}}</a>
                        </p>
                        @endif
                        @endforeach
                        @foreach ($cate as $item)
                        @if ($item->id==$movie->cate_id)
                        <p><b>Thể loại: </b><a href="{{ route('user.cate',$movie->cate_id) }}">{{$item->cate_name}}</a>
                        </p>
                        @endif
                        @endforeach
                                ...
                                <div class="comment-wrapper">
                                    <div class="panel panel-primary">
                                        ...
                                        <div class="panel-body" id="updateComment">
                                            @if (session('username_minmovies'))
                                            <form id="formComment" action="{{ route('user.postComment',$movie->id) }}" method="post">
                                                @csrf
                                            ...
                                            <ul class="media-list">
                                                <li class="media">
                                                    <div class="media-body">
                                                        ...
                                                        <a href="#" class="btn btn-success" data-toggle="modal" data-target="#editComment{{$item->id}}">Sửa</a>
                                                        <a href="{{ route('user.delComment',$item->id) }}" class="btn btn-danger" data-toggle="confirm" role="button">Xoá</a>
                                                        <br><br>
                                                        <div class="modal video-modal fade" id="editComment{{$item->id}}" tabindex="-1" role="dialog" aria-labelledby="myModal">
                                                            <div class="modal-dialog" role="document">
                                                                <div class="modal-content">
                                                                    <div class="modal-header">
                                                                        Sửa bình luận
                                                                        ...
                                                                        <div class="modal-body">
                                                                            <div class="w3_login_module">
                                                                                <div class="module form-module">
                                                                                    ...
                                                                                    <div class="form">
                                                                                        <form action="{{ route('user.editComment',$item->id) }}" method="post">
                                                                                            @csrf
                                                                                            <textarea name="comment" id="txtEditComment{{$item->id}}" class="form-control" rows="5" placeholder="Nhập bình luận" maxlength="250" required="" oninvalid="this.setCustomValidity('Có phải bạn có quên mất gì đó?')" oninput="this.setCustomValidity('')">{{ $item->comment }}</textarea><br>
                            ...
                            <div class="w3l-movie-gride-agile w3l-movie-gride-agile1">
                                ...
                                <div class="mid-1 agileits_w3layouts_mid_1_home">
                                    <div class="w3l-movie-text">
                                        <a href="{{route('user.movie',$item->id)}}" title="{{$item->vie_name.' ('.$item->eng_name.')'}}">{{$item->vie_name}}</a>
                                    </div>
